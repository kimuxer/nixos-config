...
(setopt org-directory "~/.config/doom/org/")
...
(after! org-roam
  (setopt org-roam-directory (file-truename "~/.config/doom/org/roam/"))
...
;; 只在真的找到 bash 时才覆盖 shell-file-name；找不到就保持 Emacs 自带的默认值，
;; 不再硬编码 "/bin/sh"（NixOS 上未必存在，会导致 process-file 报 file-missing），
;; 也不会把它静默设为 nil。
